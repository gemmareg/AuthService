...
namespace AuthService.Application.UnitTest.Services
{
    public class UserServiceTests
    {
        ...
        public async Task LoginAsync_ReturnsOk_WhenCredentialsAreValid()
        {
            ...
            _tokenServiceMock.Setup(p => p.GenerateAccessTokenAsync(It.IsAny<User>())).ReturnsAsync("valid-access-token");
            _tokenServiceMock.Setup(p => p.GenerateRefreshToken(It.IsAny<Guid>())).ReturnsAsync(Result<RefreshToken>.Ok(refreshToken));
        ...
        [Fact]
        public async Task SoftDeleteAsync_ReturnsOk_WhenRequesterIsDifferentUser()
        {
            // La autorización (self, o el permiso "users:delete:any" según el
            // claim del token) es responsabilidad del controller, no del
            // servicio — este ya no repite ninguna comprobación contra la BD.
